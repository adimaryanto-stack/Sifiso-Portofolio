...
export function GetStartedClient({ dbPackages }: { dbPackages: any[] }) {
  ...
  // Convert DB packages to tier objects or fall back to default tiers
  const tiers = dbPackages && dbPackages.length > 0
    ? dbPackages.map((pkg, idx) => {
        let icon = Zap;
        if (pkg.name.toLowerCase().includes("start")) icon = Target;
        else if (pkg.name.toLowerCase().includes("enter")) icon = Shield;
        else if (idx === 0) icon = Target;
        else if (idx === 2) icon = Shield;

        return {
          name: pkg.name,
          price: pkg.price,
          description: pkg.badge || `Tailored pricing package for ${pkg.name}.`,
          features: (pkg.features as string[]) || [],
          icon: icon,
          button: `Select ${pkg.name}`,
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      <select 
                        ...
                        className="w-full bg-surface-elevated/50 border border-border rounded-2xl px-6 py-5 focus:outline-none focus:border-primary transition-all duration-300 focus:bg-black text-white appearance-none cursor-pointer"
                      >
                        {tiers.map((t) => (
                          <option key={t.name} value={t.name}>
                            {t.name} Package ({t.price})
                          </option>
                        ))}
                        <option value="Custom / Individual">Custom / Individual Package</option>
                      </select>
